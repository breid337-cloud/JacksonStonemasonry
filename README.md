...
5. **Replace placeholder images** with real photography. The hero background (`src/assets/img/hero-wall.jpg`) is a procedurally rendered Cotswold stone texture — overwrite that one file with a real 1920×1080 photo and every hero on the site updates, no code changes. Project thumbnails still use `placeholder-wall.svg`, and `og-default.svg` should become a real 1200×630 JPG/PNG (update the two references in `base.njk` and `schema-localbusiness.njk`).

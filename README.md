...
1. **Set the real domain** in `src/_data/site.json` (`url`). The placeholder `jacksonsstonemasonry.co.uk` drives canonicals, the sitemap and structured data.
...
5. **Replace placeholder images** in `src/assets/img/` (all SVG mock-ups of stone coursing) with real photography: hero, project photos, and a real 1200×630 JPG/PNG `og-default` image (update the two references in `base.njk` and `schema-localbusiness.njk`).
